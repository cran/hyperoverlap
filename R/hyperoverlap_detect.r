...
hyperoverlap_detect <- function(x, y, kernel="polynomial",kernel.degree=3, cost=500, stoppage.threshold=0.4, verbose = TRUE, set = FALSE) {
  ...
  if(misclass==0){
    ...
    if (kernel =="polynomial"){
      if (misclass > stoppage.threshold*nrow(x)) {
        ...
        if(verbose) print("Misclassified points above stoppage threshold; curvilinear hyperoverlap not attempted.")
        return(methods::new("Hyperoverlap",
                            entity1=entity1,
                            entity2=entity2,
                            dimensions=dimensions,
